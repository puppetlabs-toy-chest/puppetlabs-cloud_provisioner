  ...
  describe 'option validation' do
    describe '(region)' do
      it "should not require a region name" do
        ...
        expect { subject.list_keynames(@options) }.to raise_error Exception, 'region:us-east-1'
      end

      it 'should validate the region' do
        @options[:region] = 'mars-east-100'
        expect { subject.list_keynames(@options) }.to raise_error ArgumentError, /Unknown region/
      end
    end
