...
module Puppet::CloudPack
  ...
  class << self
    ...
    def add_payload_options(action)
      ...
      action.option '--puppetagent-dflt-certname' do
        summary 'Let the puppet agent decide what the certificate name should be on the target system.'
        description <<-EOT
          This option allows you to skip autogenerated certificate name and let
          the target host decide what it should be.
        EOT
      end
    ...
    def install(server, options)
      ...
      if not options[:puppetagent_certname] and not options[:puppetagent_dflt_certname]
